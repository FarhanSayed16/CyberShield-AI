from fastapi import APIRouter, Depends, HTTPException
from typing import List
from app.api.deps import require_jwt_or_api_key, require_org_id
from app.schemas.rules import CustomRuleCreate, CustomRuleResponse
from app.db import crud_rules

router = APIRouter()


def _rule_to_response(doc) -> CustomRuleResponse:
    data = doc.model_dump(exclude={"id", "created_at"})
    return CustomRuleResponse(id=str(doc.id), **data)


@router.post("/", response_model=CustomRuleResponse)
async def create_rule(
    rule_in: CustomRuleCreate,
    principal=Depends(require_jwt_or_api_key),
):
    """Create a new custom detection rule (org-scoped)."""
    org_id = require_org_id(principal)
    doc = await crud_rules.create_rule(rule_in, org_id=org_id)
    return _rule_to_response(doc)


@router.get("/", response_model=List[CustomRuleResponse])
async def list_all_rules(
    principal=Depends(require_jwt_or_api_key),
):
    """List custom rules for the caller's organization."""
    org_id = require_org_id(principal)
    docs = await crud_rules.list_rules(org_id)
    return [_rule_to_response(doc) for doc in docs]


@router.patch("/{rule_id}/toggle", response_model=CustomRuleResponse)
async def toggle_rule(
    rule_id: str,
    is_active: bool,
    principal=Depends(require_jwt_or_api_key),
):
    """Enable or disable a custom rule."""
    org_id = require_org_id(principal)
    doc = await crud_rules.toggle_rule_status(rule_id, is_active, org_id)
    if not doc:
        raise HTTPException(status_code=404, detail="Rule not found")
    return _rule_to_response(doc)


@router.delete("/{rule_id}")
async def delete_rule(
    rule_id: str,
    principal=Depends(require_jwt_or_api_key),
):
    """Delete a custom rule."""
    org_id = require_org_id(principal)
    success = await crud_rules.delete_rule(rule_id, org_id)
    if not success:
        raise HTTPException(status_code=404, detail="Rule not found")
    return {"message": "Rule deleted"}
